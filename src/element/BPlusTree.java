...
public class BPlusTree<T extends Comparable<T>, V> {
    ...
    public static void main(String[] args) {
        BPlusTree<Integer, Index> myTree = new BPlusTree<Integer, Index>(8);

        int max = 1000000;
        long start = System.currentTimeMillis();
        for (int i = 0; i < max; i++) {
            myTree.set(i, new Index (i, i));
        }
        ...
        Map<Integer, Index> hashMap = new HashMap<Integer, Index> ();
        for (int i = 0; i < max; i++) {
            hashMap.put(i, new Index (i, i));
        }
